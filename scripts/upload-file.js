...
function displayFileInfo(file) {
    ...
    if (file) {
        ...
        var supportedTypes = ["image/jpeg", "image/png", "application/msword", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", "application/pdf"];
...
function validateForm() {
    ...
    if (fileInput.files.length === 1 && supportedTypes.includes(fileInput.files[0].type) && fileInput.files[0].size <= maxFileSize) {
        ...
        submitButton.style.background = "var(--mlin-color)";
        submitButton.addEventListener('mouseover', function() {
            this.style.backgroundColor = 'var(--mlin-dark-color)';
            this.style.transform = 'translateY(-2px)';
            this.style.cursor = 'pointer';
        });
        submitButton.addEventListener('mouseout', function() {
            this.style.backgroundColor = 'var(--mlin-color)';
            this.style.transform = '';
        });
        errorMessage.innerHTML = '';
    } else {
        submitButton.disabled = true;
        submitButton.style.background = "var(--grey)";
        submitButton.style.pointer = 'default';
